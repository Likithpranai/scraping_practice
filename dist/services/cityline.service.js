...
class CityLineService {
    constructor() {
        this.baseUrl = 'https://www.cityline.com/Events.html';
        this.browser = null;
        this.sessionStoragePath = path.join(__dirname, '../../storage/cityline_session.json');
        this.cookiesPath = path.join(__dirname, '../../storage/cityline_cookies.json');
        this.storageStatePath = path.join(__dirname, '../../storage/cityline_storage.json');
    ...
    async loadSession() {
        try {
            ...
            if (fs.existsSync(combinedAuthPath)) {
                ...
                // Validate cookies
                const citylineCookies = combinedData.cookies.filter((cookie) => cookie.domain.includes('cityline.com') ||
                ...
                if (citylineCookies.length === 0) {
                    ...
                    citylineCookies.forEach((cookie, index) => {
    ...
    async saveSession(context) {
        try {
            ...
            fs.writeFileSync(this.sessionStoragePath, JSON.stringify(this.sessionData, null, 2), 'utf8');
     ...
     */
    async checkAndBypassLoginForm(page, credentials) {
     ...
     */
    async scrapeEvents(limit = 5, credentials, useCookieAuth = true) {
...
exports.default = new CityLineService();
//# sourceMappingURL=cityline.service.js.map