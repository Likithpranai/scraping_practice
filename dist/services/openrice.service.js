...
exports.OpenRiceService = void 0;
const logger_1 = __importDefault(require("@/utils/logger"));
const playwright_1 = require("playwright");
class OpenRiceService {
    async scrapeOpenRiceHongKongRestaurants() {
        ...
        try {
            for (const url of urlsToScrape) {
                logger_1.default.info(`Scraping URL list: ${url}`);
                ...
                try {
                    ...
                    logger_1.default.info(`Found ${restaurantUrlLocators.length} resto urls for ${url}.`);
                    ...
                    logger_1.default.error(`Error scraping URL list ${url}`, { error: error.message });
                }
                finally {
                    await page.close();
                }
            }
            logger_1.default.info(`Found a total of ${allRestoUrls.length} unique restaurant URLs to scrape.`);
            const allRestaurants = [];
            for (const restaurantUrl of allRestoUrls) {
                logger_1.default.info(`Scraping restaurant details from: ${restaurantUrl}`);
                const page = await context.newPage();
                try {
                    ...
                    restaurant.introduction = await page.evaluate(() => {
                        ...
                        if (introEl) {
                            return introEl.innerText;
                        }
                        const cropDescEl = document.querySelector('div.crop-desc');
                        if (cropDescEl) {
                            const clone = cropDescEl.cloneNode(true);
                    ...
                    restaurant.paymentMethods = await page.evaluate(() => {
                        ...
                        return Array.from(paymentElements).map(el => el.innerText);
                    }).catch(() => []);
                    ...
                    logger_1.default.info(`Successfully scraped: ${restaurant.name}`);
                    ...
                    logger_1.default.error(`Error scraping restaurant details from ${restaurantUrl}`, { error: error.message });
                }
                finally {
                    await page.close();
                }
            }
            logger_1.default.info(`Successfully scraped ${allRestaurants.length} unique restaurant details from ${urlsToScrape.length} list pages.`, {
                durationMs: Date.now() - startTime,
            });
            return allRestaurants;
        }
        catch (error) {
            logger_1.default.error(`An unexpected error occurred during scraping.`, { error: error.message });
            return [];
        }
        finally {
            await browser.close();
...
exports.OpenRiceService = OpenRiceService;
exports.default = new OpenRiceService();
//# sourceMappingURL=openrice.service.js.map