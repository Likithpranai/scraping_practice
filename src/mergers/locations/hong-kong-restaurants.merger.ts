...
import { RecognitionDetail } from '@/types/common.model';
import logger from '@/utils/logger';

export class HongKongRestaurantsMerger extends IMerger<Restaurant> {
  merge(results: ScrapingResult<Restaurant>[]): Restaurant[] {
    logger.debug('Starting Hong Kong restaurants custom merge', { resultCount: results.length });
    
    const mergedMap = new Map<string, Restaurant>();
    
    // Group results by strategy
    const resultsByStrategy = new Map<string, Restaurant[]>();
    ...
    logger.debug('Completed Hong Kong restaurants custom merge', {
  ...
  private generateRestaurantKey(restaurant: Restaurant): string {
  ...
  private findCandidates(target: Restaurant, results: ScrapingResult<Restaurant>[]): Array<{restaurant: Restaurant, strategy: string}> {
    const candidates = [];
  ...
  private intelligentMerge(candidates: Array<{restaurant: Restaurant, strategy: string}>): Restaurant {
    if (candidates.length === 1) {
      return candidates[0].restaurant;
    }
    
    // Start with a deep copy of the first candidate to serve as the base
    const merged: Restaurant = JSON.parse(JSON.stringify(candidates[0].restaurant));

    // --- Accumulators for merging ---
    const hiddenGemScores: number[] = [];
    const tagsTypeAccumulator: Record<string, number[]> = {};
    const tagsGroupAccumulator: Record<string, number[]> = {};
    const allSignatureFoods = new Set<string>();
    const allClosingDays = new Set<string>();
    const allClosingDates = new Set<string>();
    const allRecognitions: Record<string, RecognitionDetail> = {};

    // --- Gather data from all candidates ---
    for (const { restaurant } of candidates) {
      // Prefer longest/most descriptive text fields
      if (restaurant.local_name && restaurant.local_name.length > (merged.local_name?.length || 0)) merged.local_name = restaurant.local_name;
      if (restaurant.address && restaurant.address.length > (merged.address?.length || 0)) merged.address = restaurant.address;
      if (restaurant.neighborhood && restaurant.neighborhood.length > (merged.neighborhood?.length || 0)) merged.neighborhood = restaurant.neighborhood;
      if (restaurant.text_embedding && restaurant.text_embedding.length > (merged.text_embedding?.length || 0)) merged.text_embedding = restaurant.text_embedding;
      if (restaurant.local_tips && restaurant.local_tips.length > (merged.local_tips?.length || 0)) merged.local_tips = restaurant.local_tips;

      // Collect scores for averaging
      if (restaurant.hidden_gem_score) hiddenGemScores.push(restaurant.hidden_gem_score);

      // Collect tags for averaging
      for (const [tag, score] of Object.entries(restaurant.tags_type || {})) {
        if (!tagsTypeAccumulator[tag]) tagsTypeAccumulator[tag] = [];
        tagsTypeAccumulator[tag].push(score);
      }
      for (const [tag, score] of Object.entries(restaurant.tags_group || {})) {
        if (!tagsGroupAccumulator[tag]) tagsGroupAccumulator[tag] = [];
    ...
    for (const [tag, scores] of Object.entries(tagsTypeAccumulator)) {
      ...
      (merged.tags_type as any)[tag] = avg;
    }
    for (const [tag, scores] of Object.entries(tagsGroupAccumulator)) {
      const avg = Math.round(scores.reduce((a, b) => a + b, 0) / scores.length);
      (merged.tags_group as any)[tag] = avg;
    }
