 ...
 */
async function retry<T>(fn: () => Promise<T>, retries = 3, delay = 10000, backoff = 2): Promise<T> {
  try {
    return await fn();
  } catch (error) {
    if (retries <= 0) {
...
async function scrapeAllPages() {
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    try {
      ...
      const result = await retry(async () => {
        return await siteContentService.getSiteContent('', 15, page);
      }, 3, 15000);
      ...
      if (result.events && result.events.length > 0) {
        ...
        fs.writeFileSync(
          path.join(resultsDir, `page-${page}-events.json`), 
      ...
      fs.writeFileSync(
        path.join(resultsDir, `partial-results-up-to-page-${page-1}.json`), 
  ...
  fs.writeFileSync(
    finalResultPath, 
    JSON.stringify({ events: allEvents, totalEvents: allEvents.length }, null, 2)
  );
  ...
  // Properly type the eventsByPage object
  interface EventsByPageMap {
    [key: string]: string[];
  }
  
  const eventsByPage: EventsByPageMap = {};
  ...
  Object.keys(eventsByPage).forEach((pageKey: string) => {
    console.log(`\nPage ${pageKey}: ${eventsByPage[pageKey].length} events`);
    eventsByPage[pageKey].forEach((title: string, i: number) => {
