...
async function scrapeCharityEvents() {
  ...
  try {
    ...
    eventFoodService.baseUrl = 'https://www.eventbrite.hk/d/hong-kong-sar/charity-and-causes--events/';
    
    // Scrape page 1 only
    console.log('Scraping charity events from page 1...');
    const charityEvents = await eventFoodService.scrapeEvents(1);
    
    // Save results
    const outputPath = path.join(__dirname, '../results/eventbrite_charity.json');
    fs.writeFileSync(outputPath, JSON.stringify(charityEvents, null, 2));
