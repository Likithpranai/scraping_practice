...
async function scrapeAdditionalPages() {
  ...
  try {
    ...
    const existingDataPath = path.join(__dirname, '../results/event_food_test.json');
    let existingData = [];
    
    if (fs.existsSync(existingDataPath)) {
      const existingDataRaw = fs.readFileSync(existingDataPath, 'utf8');
    ...
    const additionalEvents = await eventFoodService.scrapeAdditionalPages([2, 3]);
    ...
    const outputPath = path.join(__dirname, '../results/event_food_test.json');
