...
async function scrapePerformingEvents() {
  ...
  try {
    ...
    eventFoodService.baseUrl = 'https://www.eventbrite.hk/d/hong-kong-sar/arts--events/';
    
    // Scrape pages 1-3
    console.log('Scraping arts events from pages 1-3...');
    const artsEvents = await eventFoodService.scrapeEvents(3);
    
    // Save results
    const outputPath = path.join(__dirname, '../results/eventbrite_performing.json');
    fs.writeFileSync(outputPath, JSON.stringify(artsEvents, null, 2));
