...
class SiteContentService {
  ...
  private async goToNextPage(page: Page): Promise<boolean> {
    try {
      ...
      const paginationInfo = await page.evaluate(() => {
        ...
        let currentPageInput = document.querySelector('.toggle-page_input') as HTMLInputElement;
        if (!currentPageInput) currentPageInput = document.querySelector('.toggle-events-page input[type="number"]') as HTMLInputElement;
        
        // Try multiple possible selectors for total pages
        let totalPagesSpan = document.querySelector('.toggle-page_total');
        if (!totalPagesSpan) totalPagesSpan = document.querySelector('.toggle-events-page .toggle-page_sparator + span');
        ...
        try {
          ...
          const inputFieldExists = await page.evaluate(() => {
            ...
            for (const selector of selectors) {
              ...
              if (input) return { found: true, selector: selector };
            }
   ...
   */
  private async goToSpecificPage(page: Page, pageNumber: number): Promise<boolean> {
    try {
   ...
   */
  private async retryOperation<T>(operation: () => Promise<T>, maxRetries = 3, initialDelay = 5000): Promise<T> {
    let retries = 0;
   ...
   */
  public async getSiteContent(url: string = '', maxEvents: number = 50, startPage: number = 1): Promise<{ events: EventDetail[] }> {
    ...
    try {
      ...
      while (currentPage <= maxPages && events.length < maxEvents) {
        ...
        const eventsToProcess = Math.min(
          maxEvents - events.length, // Don't exceed max total
