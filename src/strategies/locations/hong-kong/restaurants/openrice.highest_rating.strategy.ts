...
export class OpenRiceHighestRatingStrategy extends IStrategy<Restaurant> {
  private readonly baseUrl = 'https://www.openrice.com/zh/hongkong/explore/chart/best-rating?_sUrl=https%3A%2F%2Fs.openrice.com%2FQrbe0000000';
  ...
  async get_info(): Promise<ScrapingResult<Restaurant>> {
    ...
    try {
      ...
      const restaurantPages = await scraperService.scrapeMultiplePages(
        restaurantLinks.map(link => link.url).slice(0, 50) // Limit to 50 for this example
      );
