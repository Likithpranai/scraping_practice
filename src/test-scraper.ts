...
async function runScraper() {
  ...
  try {
    const result = await siteContentService.getSiteContent('', 50, pageNumber);
    ...
    result.events.forEach((event, index) => {
      ...
      if (event.additionalDetails?.Website) console.log(`Website: ${event.additionalDetails.Website}`);
